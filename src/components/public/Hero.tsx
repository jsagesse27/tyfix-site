...
export default function Hero({ settings, content, soldCount }: HeroProps) {
  ...
  const headlineParts = headline.includes('$') ? [headline.split('$')[0], '$' + headline.split('$')[1]] : [headline, null];

  return (
    <>
      <section className="relative min-h-[100svh] flex flex-col justify-center bg-[#050A14] overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src={heroImage} alt="Quality used cars at TyFix Auto Sales" className="w-full h-full object-cover opacity-30" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pt-28 pb-16">
          <div className="max-w-3xl">
            ...
            <h1 className="text-5xl sm:text-6xl md:text-7xl xl:text-8xl font-black text-white leading-[1.0] tracking-tight mb-6 animate-fade-in" style={{ animationDelay: '100ms', opacity: 0, fontFamily: 'Space Grotesk, sans-serif' }}>
              {headlineParts[1] ? (<>{headlineParts[0]}<br /><span className="text-primary" style={{ textShadow: '0 0 60px rgba(139,0,0,0.4)' }}>{headlineParts[1]}</span></>) : headline}
            </h1>
          ...
          <div className="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-4 animate-fade-in" style={{ animationDelay: '450ms', opacity: 0 }}>
            {stats.map((stat, i) => (
              <div key={i} className="border border-white/10 rounded-2xl p-4 backdrop-blur-sm bg-white/5 text-center">
